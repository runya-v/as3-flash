package {
	...
	public class Restaurant extends Sprite {
		...
		public static const URL:String = "http://vi-ex.ru/restaurant/rs-api/restaurant";
		...
		private function init(e:Event = null):void {
            ...
            sloader.addEventListener(SceneLoader.LOADED_VIEWPOINT, function(e:MessageEvent):void {
                ...
                var cam:Camera = new Camera(id, args[3], args[4]);
                _cameras[id] = cam; 
            ...
            _view = new View3D({ camera:_cam_control.m_cam, x:cam_x, y:cam_y, renderer:Renderer.CORRECT_Z_ORDER });
			this.addChild(_view);
            ...
            this.addEventListener(MouseEvent.MOUSE_MOVE, function(e:MouseEvent):void {
                if (_mouse_dovn) {
                    ...
                    if (shift_x < 0) {
                        _cam_control.m_cam.moveRight(-shift_x);
                        _cam_control.m_cam.panAngle += shift_x;
                    }
                    
                    if (shift_x > 0) {
                        _cam_control.m_cam.moveLeft(shift_x);
                        _cam_control.m_cam.panAngle += shift_x;
                    }
                    
                    if (shift_y < 0) {
                        _cam_control.m_cam.moveDown(-shift_y);
                        _cam_control.m_cam.tiltAngle += shift_y
                    }
                    
                    if (shift_y > 0) {
                        _cam_control.m_cam.moveUp(shift_y);
                        _cam_control.m_cam.tiltAngle += shift_y;
                    }
